...
RUN sed -i '2s/^/#set observer password\necho "setting observer password"\n\/home\/openbis\/openbis\/servers\/openBIS-server\/jetty\/bin\/passwd.sh add -p ${OBSERVER_PASS} observer\n /' /usr/local/bin/docker-entrypoint.sh
